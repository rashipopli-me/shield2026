...
| TEST MODE
|--------------------------------------------------------------------------
| Set TEST_MODE=true in .env when you want to dry-run attendance.
|
| TEST MODE:
| - Treats today as Day 1
| - Allows attendance scanning outside the real event dates
|
| REAL MODE:
| - Uses the actual EVENT_START_DATE / EVENT_DAYS settings
|--------------------------------------------------------------------------
*/

const TEST_MODE = process.env.TEST_MODE === "true";

function getAttendanceEventContext() {
  if (TEST_MODE) {
...
router.post("/checkin", requireAuth, async (req, res) => {
  try {
    ...
    const forcedSession = ["morning", "evening"].includes(req.body.session)
      ? req.body.session
      : null;
    ...
    if (!ticket) {
      return res.status(404).json({
        ...
        message: "This QR code doesn't match any issued ticket.",
      });
    }
    ...
    if (!ticket.participant) {
      return res.status(409).json({
        error: "UNAUTHORIZED",
        message: "This ticket isn't linked to a registered participant.",
      });
    }

    // TEST MODE uses today as Day 1.
    // REAL MODE uses the actual configured event dates.
    const { day, date, withinEventWindow } =
      getAttendanceEventContext();

    if (!withinEventWindow) {
      return res.status(409).json({
        error: "OUTSIDE_EVENT_WINDOW",
        message:
          "Today isn't within the configured bootcamp dates. Check EVENT_START_DATE / EVENT_DAYS in the server config.",
      });
    }
    ...
    res.status(500).json({
      error: "SERVER_ERROR",
      message: "Something went wrong recording attendance.",
    });
  }
});
...
router.get("/today", requireAuth, async (req, res) => {
  const { day, date } = getAttendanceEventContext();
...
router.get("/all", requireAuth, async (req, res) => {
  ...
  const totalDays = Number(
    process.env.EVENT_DAYS || 5
  );

  const perDay = [];

  for (let day = 1; day <= totalDays; day++) {
    const morning = await Attendance.countDocuments({
      day,
      session: "morning",
    });

    const evening = await Attendance.countDocuments({
      day,
      session: "evening",
    });
...
router.get("/export.csv", requireAuth, async (req, res) => {
  ...
  const header =
    "UID,Name,Official Email,Phone Number,Branch,Day,Session,Date,CheckInTime,Status\n";
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=shield2026_attendance.csv"
  );
